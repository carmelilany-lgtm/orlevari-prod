...
**SEO:** Edit `seo_title_en` / `seo_description_en` (and the Hebrew pair) in content admin. English is the default URL (`/en`); Hebrew is `/he`. Set `NEXT_PUBLIC_SITE_URL` for canonical/Open Graph/sitemap. Optional `GOOGLE_SITE_VERIFICATION` for Search Console HTML-tag verification.
