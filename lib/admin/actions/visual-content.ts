...
export async function saveVisualContentUpdates(
  ...
  fieldStylesJson?: string | null,
): Promise<ActionResult> {
  const ctx = await requireAdmin();
  if (!("supabase" in ctx)) return ctx;

  const hasText = updates.length > 0;
  const hasStyles = fieldStylesJson !== undefined;
  ...
  if (hasStyles) {
    const canonical = stringifyVisualFieldStyles(
      parseVisualFieldStyles(fieldStylesJson),
    );
    const { error } = await ctx.supabase.from("site_content").upsert(
      {
        key: VISUAL_FIELD_STYLES_CONTENT_KEY,
        value_en: canonical,
        value_he: canonical,
      },
      { onConflict: "key" },
    ...
    if (error) {
      return actionError(
        adminErrors.saveContentFailed(VISUAL_FIELD_STYLES_CONTENT_KEY, error.message),
      );
    }
  }
